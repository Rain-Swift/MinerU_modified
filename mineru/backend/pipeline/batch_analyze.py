    ...
    def __call__(self, images_with_extra_info: list) -> list:
        ...
        if self.table_enable:
            for table_res_dict in tqdm(table_res_list_all_page, desc="Table Predict"):
                ...
                html_code, table_cell_bboxes, logic_points, elapse, table_text_with_bbox = table_model.predict(table_res_dict['table_img'])
                # 判断是否返回正常
                if html_code:
                    ...
                    if expected_ending:
                        ...
                        # 定义辅助函数，用于将各种类型（包括NumPy类型）转换为Python原生类型
                        ...
                        # 保存表格文本和边界框信息
                        if table_text_with_bbox is not None:
                            # 确保所有表格文本和边界框信息都是Python原生类型
                            def convert_text_with_bbox(items):
                                result = []
                                for item in items:
                                    native_item = {}
                                    for key, value in item.items():
                                        if isinstance(value, (np.integer, np.floating)):
                                            native_item[key] = float(value)
                                        elif isinstance(value, np.ndarray):
                                            native_item[key] = value.tolist()
                                        elif isinstance(value, (list, tuple)):
                                            native_item[key] = convert_to_native_types(value)
                                        else:
                                            native_item[key] = value
                                    result.append(native_item)
                                return result
                            
                            native_text_with_bbox = convert_text_with_bbox(table_text_with_bbox)
                            table_res_dict['table_res']['table_cell_bboxes'] = native_text_with_bbox
