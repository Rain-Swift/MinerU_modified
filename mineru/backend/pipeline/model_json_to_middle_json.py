...
def result_to_middle_json(model_list, images_list, pdf_doc, image_writer, lang=None, ocr_enable=False, formula_enabled=True):
    ...
    for page_index, page_model_info in tqdm(enumerate(model_list), total=len(model_list), desc="Processing pages"):
        ...
        if 'preproc_blocks' in page_info:
            for block in page_info['preproc_blocks']:
                if block['type'] == 'table' and 'blocks' in block:
                    for sub_block in block['blocks']:
                        if sub_block['type'] == 'table_body' and 'lines' in sub_block:
                            for line in sub_block['lines']:
                                if 'spans' in line:
                                    for span in line['spans']:
                                        if span['type'] == 'table' and 'html' in span:
                                            # 为每个表格创建table_texts字段，无论是否存在table_text_with_bbox
                                            table_texts = []
                                            
                                            # 确保从block中继承表格的bbox
                                            if 'bbox' in block and 'bbox' in span:
                                                span['ocr_bbox'] = to_native_type(block['bbox'])
                                                
                                            # 保存表格本身的bbox
                                            if 'ocr_bbox' in span:
                                                span['table_bbox'] = span['ocr_bbox']
                                            
                                            # 检查是否存在表格文本信息
                                            if 'table_cell_bboxes' in span and span['table_cell_bboxes']:
                                                for text_item in span['table_cell_bboxes']:
                                                    try:
                                                        # 确保所有数值都是Python原生类型
                                                        text = text_item.get('text', '')
                                                        bbox = to_native_type(text_item.get('bbox', [0.0, 0.0, 0.0, 0.0]))
                                                        polygon = to_native_type(text_item.get('polygon', []))
                                                        confidence = to_native_type(text_item.get('confidence', 0.0))
                                                        ocr_hw = text_item.get('ocr_hw', ())
                                                    except Exception as e:
                                                        logger.error(f"处理表格文本时发生错误: {e}")
                                                        continue
                                                ...
                                                # 移除原始的表格文本信息，减少冗余
                                                span.pop('table_cell_bboxes', None)
                                            
                                            # 将表格文本信息添加为新的字段，即使为空
                                            span['table_texts'] = table_texts
    ...
    # 注意：字符级bbox转换已移至后置处理，在markdown生成后进行
