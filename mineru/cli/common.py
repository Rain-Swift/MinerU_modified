...
from mineru.utils.extract_text_bbox import extract
from mineru.utils.post_char_bbox_converter import convert_middle_json_to_char_level
    ...
    if backend == "pipeline":
        ...
        for idx, model_list in enumerate(infer_results):
            ...
            if f_dump_middle_json:
                # 输出行级middle.json
                md_writer.write_string(
                    f"{pdf_file_name}_middle2.json",
                    json.dumps(middle_json, ensure_ascii=False, indent=4),
                )
                
                # 使用新的后置字符级bbox转换器
                # 如果md_content_str为None，使用空字符串
                md_content_for_conversion = md_content_str if md_content_str is not None else ""
                char_level_mapping = convert_middle_json_to_char_level(middle_json, md_content_for_conversion)
                md_writer.write_string(
...
if __name__ == "__main__":
    ...
    pdf_path = "/home/czr/MinerU_2/使用说明.pdf"
